...
data IncomingApiEntry = IncomingApiEntry
  {
    apiRequest  :: IncomingApiRequest,
    apiResponse :: IncomingApiResponse,
    apiTag      :: Text,
    hostname    :: Text,
    timestamp   :: A.Value
  }
...
data IncomingApiRequest = IncomingApiRequest
  { 
    apiReqBody        :: A.Value,
    apiReqUrl         :: Text,
    apiReqMethod      :: Text,
    apiReqHeaders     :: HM.HashMap Text Text,
    apiReqQueryParams :: HM.HashMap Text Text,
    apiReqRouteParams :: HM.HashMap Text Text
  }
  deriving stock (Show, Generic)
  deriving anyclass (ToJSON, FromJSON)

data IncomingApiResponse = IncomingApiResponse
  { 
    apiResBody    :: A.Value,
    apiResHeaders :: HM.HashMap Text Text,
    apiResCode    :: Int
  }
  deriving stock (Show, Generic)
  deriving anyclass (ToJSON, FromJSON)
...
data RecordingEntry
  ...
  | TimeStampEntryT TimeStampEntry
  | RandomRIOEntryT RandomRIOEntry
  | RandomBytesEntryT RandomBytesEntry
  | RunIOWithArtEntryT RunIOWithArtEntry
  | GlobalOptionsEntryT GlobalOptionsEntry
  | ARTConfigEntryT ARTConfigEntry
  | UuidEntryT UuidEntry
  deriving stock (Show, Generic)
  deriving anyclass (ToJSON,FromJSON)

data UuidEntry = UuidEntry
  {
    functionName  :: Text,
    uuid          :: A.Value
  }
...
data TimeStampEntry = TimeStampEntry
  {
    functionName  :: Text,
    timestamp     :: A.Value
  }
...
data RandomRIOEntry = RandomRIOEntry
  {
    functionName  :: Text,
    range         :: A.Value,
    value         :: A.Value,
...
data RandomBytesEntry = RandomBytesEntry
  {
    functionName  :: Text,
    input         :: Int,
    value         :: A.Value,
...
data RunIOWithArtEntry = RunIOWithArtEntry
  {
    functionName  :: Text,
    input         :: [A.Value],
    value         :: A.Value,
...
data GlobalOptionsEntry = GlobalOptionsEntry
  {
    functionName  :: Text,
    key :: Text,
    value :: Maybe A.Value
...
data ARTConfigEntry = ARTConfigEntry
  {
    functionName  :: Text,
    key :: Text,
    value :: Maybe A.Value
...
data InMemEntryValue = InMemEntryValue
  {
    ttl     :: POSIXTime,
    jsonVal :: A.Value
  }
...
data RunKVDBEntry =
  ...
  | RZAddBT RZAddB
  | RZRangeBT RZRangeB
  | RZRangeWithScoresBT RZRangeWithScoresB
  | RZRangeByScoreWithScoresBT RZRangeByScoreWithScoresB
  | RZRangeByScoreWithScoreBT RZRangeByScoreWithScoreB
  | RLRangeBT RLRangeB
  | RLPushBT RLPushB
  | RRPopBT RRPopB
  | RLLenBT RLLenB
...
data RZRangeWithScoresB = RZRangeWithScoresB{
      key :: ByteString
    , startRank :: Integer 
    , stopRank :: Integer
    , jsonResult :: Either A.Value A.Value
...
data RZRangeByScoreWithScoresB = RZRangeByScoreWithScoresB{
      key :: ByteString
    , minScore :: Double 
    , maxScore :: Double
    , jsonResult :: Either KVDBReply [(L.KVDBValue,Double)]
    , timestamp :: LocalTime
    , redisName :: Text
  }
  deriving stock (Show, Generic)
...
data RZRangeByScoreWithScoreB = RZRangeByScoreWithScoreB
  { key :: ByteString,
    minScore :: Double,
    maxScore :: Double,
    jsonResult :: Either A.Value Integer,
...
data RIncrByFloatB = RIncrByFloatB {
    key :: ByteString
    , value      :: Double
    , jsonResult :: Either A.Value A.Value
    , timestamp  :: LocalTime
...
$(MBA.deriveSerialize [d|instance MBA.Serialize WaiRequest|])
instance T.OptionEntity WaiRequest HTTPRequest

data ShouldRecordSession = ShouldRecordSession
  deriving stock (Show, Generic)
  deriving anyclass (ToJSON)

$(MBA.deriveSerialize [d|instance MBA.Serialize ShouldRecordSession|])
instance T.OptionEntity ShouldRecordSession Bool
