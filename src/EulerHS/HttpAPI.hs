...
module EulerHS.HttpAPI
    ...
    , mkHttpApiCallLogEntryWithoutMasking
    , shouldBypassProxy
    , withOptionalHeaders
    ) where
...
data EncryptedHttpApiCallLogEntry = EncryptedHttpApiCallLogEntry
  { req_body    :: Maybe A.Value
  , res_body    :: Maybe A.Value
  }
  deriving stock (Show,Generic)

instance ToJSON EncryptedHttpApiCallLogEntry where
  toJSON = A.genericToJSON A.defaultOptions { A.omitNothingFields = True }
...
mkHttpApiCallLogEntryWithoutMasking :: HTTPRequest -> HTTPResponse -> EncryptedHttpApiCallLogEntry
mkHttpApiCallLogEntryWithoutMasking req res = EncryptedHttpApiCallLogEntry
  { 
    req_body = (\x -> (fromMaybe (A.String "") . A.decode . T.getLBinaryString) <$> x.getRequestBody) req
  , res_body = (\x -> (A.decode . T.getLBinaryString) x.getResponseBody) res
  }
...
hasHeader :: HeaderName -> HTTPRequest -> Bool
hasHeader headerName (HTTPRequest {getRequestHeaders}) = 
  maybe False (const True) $ Map.lookup headerName getRequestHeaders
...
-- This function won't overwrite any header, it will just add headers if not already present
withOptionalHeaders :: [(HeaderName, HeaderValue)] -> HTTPRequest -> HTTPRequest
withOptionalHeaders additionalHeaders request@HTTPRequest {getRequestHeaders} =
  let headers = foldl (\hMap (hName, hValue) -> Map.insertWith (\_ old -> old) hName hValue hMap) getRequestHeaders additionalHeaders
    in request { getRequestHeaders = headers }
...
type MaskReqRespBody = HttpData -> Maybe Text -> Maybe Text -> A.Value

maskHTTPRequest :: Maybe Log.LogMaskingConfig -> HTTPRequest -> Maybe MaskReqRespBody -> Maybe Text -> Maybe Text -> HTTPRequestMasked
maskHTTPRequest mbMaskConfig request mbMaskReqBody mbEncKey mbIv = HTTPRequestMasked
  ...
  where
    ...
    maskedRequestBody =
      case mbMaskReqBody of
        Just mskReqBodyFn -> Just $ mskReqBodyFn (HttpRequest request) mbEncKey mbIv
        Nothing           -> parseRequestResponseBody (shouldMaskKey mbMaskConfig) getMaskText mbEncKey mbIv (getContentTypeForHTTP requestHeaders) . LB.toStrict . T.getLBinaryString <$> requestBody

maskHTTPResponse :: Maybe Log.LogMaskingConfig -> HTTPResponse -> Maybe MaskReqRespBody -> Maybe Text -> Maybe Text -> HTTPResponseMasked
maskHTTPResponse mbMaskConfig response mbMaskResBody mbEncKey mbIv = HTTPResponseMasked
  ...
  where
    ...
    maskedResponseBody =
      case mbMaskResBody of
        Just mskResBodyFn -> mskResBodyFn (HttpResponse response) mbEncKey mbIv
        Nothing           -> parseRequestResponseBody (shouldMaskKey mbMaskConfig) getMaskText mbEncKey mbIv (getContentTypeForHTTP responseHeaders) . LB.toStrict . T.getLBinaryString $ responseBody

httpBypassProxyList :: Maybe Text
httpBypassProxyList =
    let envType =  Env.JuspayEnv
                    { key = "HTTP_PROXY_BYPASS_LIST"
