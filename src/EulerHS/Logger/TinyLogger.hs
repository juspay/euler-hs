...
import           EulerHS.Logger.Types (BufferSize, FlowFormatter,
                                       ...
                                       PendingMsg (..), Message(..), ErrorL(..), ExceptionEntry(..), getFlowGuuid, getLogLevel, getLogContext, getMessageNumber, getErrorLogFromException, getARTStatus)
  ...
  where
    makeErrorLog e pMsg =
      V2
        ...
        Nothing
        Nothing
        (Message (Just $ A.toJSON $ ((show e) :: Text) ) Nothing)
        (getMessageNumber pMsg)
        (getLogContext pMsg)
        (getARTStatus pMsg)
    errorLog e = 
      let expEntry = getErrorLogFromException e
        in ErrorL (Just $ jp_error_code expEntry) (error_code expEntry) (Text.pack $ error_message expEntry)
