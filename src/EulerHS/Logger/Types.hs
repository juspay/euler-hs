...
module EulerHS.Logger.Types
    ...
    , getTrackingIdsFromLogContext
    , getTrackingIdKeyHeaderMap
    ) where
...
data PendingMsg =
  ...
  | V2 !(Maybe T.FlowGUID) !LogLevel !Category !(Maybe Action) !(Maybe PrimaryKey) !(Maybe SecondaryKeys) !(Maybe Entity) !(Maybe ErrorL) !(Maybe Latency) !(Maybe T.IOLatencyMetric) !(Maybe Int64) !(Maybe Int32) !(Maybe Int64) !(Maybe RespCode) !Message !MessageNumber !LogContext !(Maybe Bool)
  deriving (Show)

type Category = Text
type Action = Text
type PrimaryKey = A.Value
type SecondaryKeys = A.Value
type Entity = Text
...
defaultMessageFormatter (V2 _ lvl category action _ _ _ _ _ _ _ _ _ _ msg _ _ _) =
...
data VersionLoggerMessage = 
    Ver1 !Tag !Message
  | Ver2 !Category !(Maybe Action) !(Maybe PrimaryKey) !(Maybe SecondaryKeys) !(Maybe Entity) !(Maybe ErrorL) !(Maybe Latency) !(Maybe T.IOLatencyMetric) !(Maybe Int64) !(Maybe Int32) !(Maybe Int64) !(Maybe RespCode) !Message !(Maybe Bool)

getFlowGuuid :: PendingMsg -> Maybe T.FlowGUID
getFlowGuuid (V1 mbFlowGuid _ _ _ _ _)           = mbFlowGuid
getFlowGuuid (V2 mbFlowGuid _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _) = mbFlowGuid

getLogLevel :: PendingMsg -> LogLevel
getLogLevel (V1 _ lvl _ _ _ _) = lvl
getLogLevel (V2 _ lvl _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _) = lvl

getLogContext :: PendingMsg -> LogContext
getLogContext (V1 _ _ _ _ _ lContext) = lContext
getLogContext (V2 _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ lContext _) = lContext

getMessageNumber :: PendingMsg -> MessageNumber
getMessageNumber (V1 _ _ _ _ msgNumber _) = msgNumber
getMessageNumber (V2 _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ msgNumber _ _) = msgNumber

getARTStatus :: PendingMsg -> Maybe Bool
getARTStatus (V1 _ _ _ _ _ _) = Nothing
getARTStatus (V2 _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ status) = status
...
convertToPendingMsg mbFlowGuid logLevel msgNum lContext (Ver2 category action primaryKey secondaryKeys entity maybeEror maybeLatency maybeIOLatencyMetric maybeCpuTime maybeMemBytes maybeGcTime maybeRespCode msg artStatus) =
    V2 mbFlowGuid logLevel category action primaryKey secondaryKeys entity maybeEror maybeLatency maybeIOLatencyMetric maybeCpuTime maybeMemBytes maybeGcTime maybeRespCode msg msgNum lContext artStatus
...
getTrackingIdsFromLogContext :: IORef LogContext -> IO [(Text, Text)]
getTrackingIdsFromLogContext ref = do
  logContext <- readIORef ref
  pure . catMaybes $ fmap (\(key1, key2, headerName) -> lookupLC key1 key2 headerName logContext) getTrackingIdKeyHeaderMap
  where
    lookupLC k1 k2 l logContext = ((,) l) <$> (lookup k1 logContext <|> (bool (Just k2) Nothing (k2 == "") >>= flip lookup logContext))

getTrackingIdKeyHeaderMap :: [(Text, Text, Text)]
getTrackingIdKeyHeaderMap =
    let envType =  Env.JuspayEnv
                    { key = "TRACKING_ID_LOGGER_KEY_HEADER_MAP"
                    , actionLeft = Env.mkDefaultEnvAction ([] :: [(Text, Text, Text)])
                    , decryptFunc = pure
                    , logWhenThrowException = Nothing
                    }
    in Env.lookupEnv envType

data LogEncryptionKey = LogEncryptionKey
  deriving stock (Eq, Show, Generic, Ord)
  deriving anyclass (ToJSON, FromJSON)

$(MBA.deriveSerialize [d|instance MBA.Serialize LogEncryptionKey|])
instance T.OptionEntity LogEncryptionKey Text

data LogEncryptionIV = LogEncryptionIV
  deriving stock (Eq, Show, Generic, Ord)
  deriving anyclass (ToJSON, FromJSON)

$(MBA.deriveSerialize [d|instance MBA.Serialize LogEncryptionIV|])
instance T.OptionEntity LogEncryptionIV Text
