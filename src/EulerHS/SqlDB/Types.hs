...
withTransaction conn f = tryAny $ case conn of
  ...
  MockingPool _-> throw $ MockingError "withTransaction called"
  where
...
bemToNative = \case
  ...
  MockingPool _       -> NativeMockingPool
...
mkSqlConn = \case
  ...
  MockingConf connTag _ _ -> pure $ MockingPool connTag
...
data ModelDBConfig (beM :: Type -> Type) = WithFallbackDB (DBConfig beM) (DBConfig beM) | WithoutFallbackDB (DBConfig beM)
...
data DBConfig (beM :: Type -> Type)
  ...
  | MockingConf ConnTag SQliteDBname PoolConfig
  -- ^ config for 'Pool' with SQlite connections
...
-- | Create Mocking 'DBConfig'
mkMockingConfig :: ConnTag -> SQliteDBname -> DBConfig a
mkMockingConfig connTag dbName = MockingConf connTag dbName defaultPoolConfig

-- | Create Mocking 'Pool' 'DBConfig'
mkMockingPoolConfig :: ConnTag -> SQliteDBname -> PoolConfig -> DBConfig a
mkMockingPoolConfig = MockingConf
...
getSchemaName :: DBConfig beM -> Text
getSchemaName = T.pack . \case
  PostgresPoolConf _ PostgresConfig{..} _ -> schemaName
  MySQLPoolConf _ MySQLConfig{..} _       -> connectDatabase -- In MySQL schema and database are same
  SQLitePoolConf _ dbName _               -> dbName
  MockingConf _ dbName _                  -> dbName

modifySchemaName :: String -> DBConfig beM -> DBConfig beM
modifySchemaName db = \case
  PostgresPoolConf a conf b -> PostgresPoolConf a (conf {schemaName = db}) b
  MySQLPoolConf a conf b    -> MySQLPoolConf a (conf {connectDatabase = db}) b
  SQLitePoolConf a _ b -> SQLitePoolConf a (db) b
  MockingConf a _ b -> MockingConf a (db) b
...
nativeToBem connTag = \case
  ...
  NativeMockingPool -> MockingPool connTag

data MockingError = MockingError Text
