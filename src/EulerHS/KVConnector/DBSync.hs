{- |
Module      :  EulerHS.KVConnector.DBSync
...
import           EulerHS.PIIEncryption(PIIMetadata(..))
...
getCreateQuery :: (ToJSON (table Identity)) => Text -> DBCommandVersion -> PIIMetadata -> Double -> DBName -> table Identity -> A.Value
...
getUpdateQuery :: DBCommandVersion -> PIIMetadata -> Double -> DBName -> A.Value -> A.Value
...
getDbUpdateCommandJsonWithPrimaryKey :: forall be beM table. (KVConnector (table Identity), Model be table, MeshMeta be table, A.ToJSON (table Identity)) => DBConfig beM -> Text -> [(Text, A.Value)] -> table Identity -> Where be table -> A.Value
...
whereClauseJsonWithPrimaryKey :: forall be beM table. (HasCallStack, KVConnector (table Identity), A.ToJSON (table Identity), MeshMeta be table) => DBConfig beM -> table Identity -> A.Value -> A.Value
...
getDbDeleteCommandJsonWithPrimaryKey :: forall be table beM. (HasCallStack, KVConnector (table Identity), Model be table, MeshMeta be table, A.ToJSON (table Identity)) => DBConfig beM -> Text -> table Identity -> Where be table -> A.Value
