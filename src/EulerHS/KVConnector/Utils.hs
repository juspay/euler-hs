{- |
Module      :  EulerHS.KVConnector.Utils
...
secondaryKeysFiltered _ table = filter filterEmptyValues (secondaryKeys  table)
  where
...
getPKeyAndValueList _ table = do
  let (PKey k) = primaryKey table
...
unsafeSetPrimaryIdInTableRow _ tableRow primaryId = do
  let (PKey p) = primaryKey tableRow
  case p of
    [(field, _)] ->
