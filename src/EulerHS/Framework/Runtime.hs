...
import           EulerHS.SqlDB.Types (ConnTag,
                                      NativeSqlPool (NativeMySQLPool, NativePGPool, NativeSQLitePool, NativeMockingPool))
...
data FlowRuntime = FlowRuntime
  ...
  , _options                  :: MVar (Map OptionKey Any)
  -- ^ Typed key-value storage
  , _optionsLocal             :: MVar (Map OptionKey Any)
  ...
  , _configCache              :: IORef (SimpleLRU.LRU W8Arr ConfigEntry)

  , _configCacheFindAll       :: IORef (SimpleLRU.LRU W8Arr ConfigEntry)

  , _configCacheLock          :: MVar (CMap.Map W8Arr ())

  , _snowflakeGenerator       :: SnowflakeGenerator
  , _recordingLocal           :: MVar ([RecordingEntry])
  , _artCounter               :: MVar (Int)
  , _kafkaProducer            :: MVar KafkaSettings
  -- ^ Connection being used for Kafka
  }
...
data KafkaSettings = KafkaSettings 
  {
    retryCount    :: Int,
    kafkaProducer :: Either KafkaError KafkaProducer,
    encKey        :: Maybe Text,
    encIV         :: Maybe Text,
    isKmsDecoded  :: Bool
  }

emptyKafkaSettings :: KafkaSettings
emptyKafkaSettings = 
  KafkaSettings 
  {
    retryCount    = 0,
    kafkaProducer = Left $ KafkaError "Connection not Initialized",
    encKey        = Nothing,
    encIV         = Nothing,
    isKmsDecoded  = False
  }
...
configCacheSize =
    let envType = Env.JuspayEnv
            ...
            , actionLeft = Env.mkDefaultEnvAction (4096 :: Integer)
            , decryptFunc = pure
            , logWhenThrowException = Nothing
            }
    in Env.lookupEnv envType

configCacheFindAllSize :: Integer
configCacheFindAllSize =
    let envType =  Env.JuspayEnv
                    { key = "CONFIG_CACHE_FINDALL_SIZE"
                    , actionLeft = Env.mkDefaultEnvAction (4096 :: Integer)
                    , decryptFunc = pure
                    , logWhenThrowException = Nothing
                    }
...
createFlowRuntime coreRt = do
  ...
  configCacheVarFindAll <- newIORef $ SimpleLRU.newLRU $ Just configCacheFindAllSize
  ...
  artCounter            <- newMVar $ (0 :: Int)
  pubSubController      <- RD.newPubSubController [] []
  kafkaProducer         <- newMVar emptyKafkaSettings
  pure $ FlowRuntime
    { _coreRuntime              = coreRt
    ...
    , _configCacheFindAll       = configCacheVarFindAll
...
clearFlowRuntime FlowRuntime{..} = do
  ...
  kafkaSettings <- takeMVar _kafkaProducer
  putMVar _kafkaProducer emptyKafkaSettings
  either (const $ pure ()) (\kafkaProducer -> closeProducer kafkaProducer) kafkaSettings.kafkaProducer
