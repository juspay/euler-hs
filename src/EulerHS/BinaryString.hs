...
instance ToJSON BinaryString where
  toJSON val = toJSON ((decodeUtf8 $ getBinaryString val) :: Text)
...
instance ToJSON LBinaryString where
  toJSON val = toJSON (decodeUtf8 (getLBinaryString val) :: Text)

instance FromJSON LBinaryString where
  parseJSON val = do
    lbs <- parseJSON val 
    pure $ LBinaryString $ Lazy.fromStrict $ encodeUtf8 @String @ByteString lbs
