...
spec = do
  ...
  describe "unaryRecordOptions" $ do
    ...
    it "Default option. Incomplete json" $ do
      eitherDecode @Cosmos enc_planetIncomplete `shouldBe` Left new_decode_error
...
new_decode_error :: String
new_decode_error = "expected Object with key \"tag\" containing one of [\"Solar\",\"Lunar\"], ErrorResp {errorType = MISSING_FIELD, errField = Just \"tag\", objectType = Nothing, errMessage = Nothing, expectedValue = Nothing, actualValue = Nothing, jPath = Nothing}"
