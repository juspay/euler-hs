...
spec =
  describe "Outgoing API call log masking" $ do
    it "Should Mask All the blackListed Keys" $ do
      ...
      let maskedValue = CType.parseRequestResponseBody (CType.shouldMaskKey mbMaskConfig) maskText Nothing Nothing Nothing (LBS.toStrict rawRequest)
      maskedValue `shouldBe` expectedOutput
    it "Should Mask All the blackListed Keys" $ do
      let rawRequest = inputJSON
      let maskText = "$**$"
      let mbMaskConfig = Just $ makeLogMaskingConfig CType.WhiteListKey ["id", "url1","a"] maskText
      let maskedValue = CType.parseRequestResponseBody (CType.shouldMaskKey mbMaskConfig) maskText Nothing Nothing (Just (encodeUtf8 ("application/json" :: Text))) (LBS.toStrict rawRequest)
      maskedValue `shouldBe` expectedOutput'
    it "Should Not Mask Any Keys" $ do
      ...
      let maskedValue = CType.parseRequestResponseBody (CType.shouldMaskKey mbMaskConfig) maskText Nothing Nothing Nothing (LBS.toStrict rawRequest)
      maskedValue `shouldBe` expectedOutput''
    it "Should Not Mask Any Keys for HTML content type" $ do
      let rawRequest = inputJSON
      let maskText = "$**$"
      let mbMaskConfig = Nothing
      let maskedValue = CType.parseRequestResponseBody (CType.shouldMaskKey mbMaskConfig) maskText Nothing Nothing (Just (encodeUtf8 ("application/html" :: Text))) (LBS.toStrict rawRequest)
      maskedValue `shouldBe` expectedOutput''
