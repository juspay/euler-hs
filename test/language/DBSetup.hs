...
userEMod :: Text -> B.EntityModification (B.DatabaseEntity be db) be (B.TableEntity UserT)
userEMod schema = B.modifyTableFields userTMod <> setEntitySchema (Just schema)
...
userDB :: Text -> B.DatabaseSettings be UserDB
userDB schema = B.defaultDbSettings `B.withDbModification`
  B.dbModification
    { users = userEMod schema
    }
