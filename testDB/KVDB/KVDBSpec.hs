    ...
    describe "EulerHS KVDB tests" $ do
      ...
      it "Double connection initialization should fail" $ \rt -> do
        eRes :: Either String () <- runFlow rt $ do
          ...
          pure $ case (eConn1, eConn2) of
            (Left err, _) -> Left $ "Failed to connect 1st time: " <> show err
            (_, Left (T.KVDBError T.KVDBConnectionAlreadyExists _)) -> Right ()
            (_, Left err) -> Left $ "Unexpected error type on 2nd connect: " <> show err
